...
namespace BookStore.API.Controllers
{
    ...
    public class CategoriesController : MainController
    {
        ...
        public async Task<IActionResult> Add(CategoryAddDto categryDto)
        {
            if (!ModelState.IsValid) return BadRequest();

            var category = _mapper.Map<Category>(categryDto);
        ...
        public async Task<ActionResult<List<Category>>> Search(string category)
        {
            ...
            if (categories == null || categories.Count == 0) return NotFound("None category was founded");
